...
import { ALL_REVIEWS, censorName } from "./data/reviews";

const PRODUCTS = [
  { id: 1, img: "https://i.imgur.com/pu7rtXm.jpeg" },
  { id: 2, img: "https://i.imgur.com/8biysCj.jpeg" },
  { id: 3, img: "https://i.imgur.com/ywhZaiD.jpeg" },
  { id: 4, img: "https://i.imgur.com/80vJXK4.jpeg" },
];



const TICKER = ["Sparepart Industri", "✦", "Pesanan Custom", "✦", "Sparepart Industri", "✦", "Pesanan Custom", "✦", "Sparepart Industri", "Pesanan Custom", "Sparepart Industri", "Pesanan Custom", "✦"];
...
const WA_SVG = ({ size = 20, color = "currentColor" }) => (
  ...
  return (
    <section id="hero" style={{ minHeight: "100vh", background: "#181818", position: "relative", overflow: "hidden", display: "flex", alignItems: "center" }}>
      ...
      <div style={{ position: "absolute", bottom: 24, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, zIndex: 3 }}>
        ...
        <div style={{ width: 1, height: 40, background: "linear-gradient(180deg,#D4520A,transparent)", animation: "bounceY 1.5s ease-in-out infinite" }} />
      </div>
    </section>
  );
}
...
function Ticker() {
  return (
    <div style={{ background: "#D4520A", padding: "12px 0", overflow: "hidden" }}>
  ...
  return (
    <section id="products" style={{ background: "#181818", padding: mobile ? "60px 24px" : "90px clamp(24px,5vw,60px)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="section-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 52, flexWrap: "wrap", gap: 20 }}>
            <div>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 10, fontSize: "0.65rem", letterSpacing: "0.22em", textTransform: "uppercase", color: "#D4520A", fontWeight: 600, marginBottom: 14 }}>
                <span style={{ width: 22, height: 1, background: "#D4520A", display: "block" }} /> Produk Kami
              </div>
              <h2 style={{ fontFamily: "Georgia,serif", fontSize: "clamp(2.2rem,4vw,3.4rem)", fontWeight: 900, color: "#fff", lineHeight: 1.1 }}>
                Apa yang <span style={{ color: "#D4520A", fontStyle: "italic" }}>Kami Buat</span>
              </h2>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isSmall ? "1fr" : "repeat(2,1fr)", gap: mobile ? 6 : 2 }}>
          {PRODUCTS.map((p, i) => (
            <FadeUp key={p.id} delay={i * 0.07}>
              <div style={{ position: "relative", overflow: "hidden", cursor: "pointer", background: "#2c2c2c", transform: hovered === p.id ? "scale(1.015)" : "scale(1)", transition: "transform 0.35s ease", zIndex: hovered === p.id ? 2 : 1 }}
                ...
                <div style={{ overflow: "hidden", height: mobile ? 200 : 270 }}>
                  <img src={p.img} alt={p.tag} style={{ width: "100%", height: "100%", objectFit: "cover", opacity: hovered === p.id ? 0.8 : 0.55, transform: hovered === p.id ? "scale(1.07)" : "scale(1)", transition: "all 0.6s ease", display: "block" }} />
                </div>
                ...
                <div style={{ position: "absolute", top: 14, left: 14, background: "#D4520A", color: "#fff", fontSize: "0.6rem", letterSpacing: "0.15em", textTransform: "uppercase", padding: "4px 10px", fontWeight: 700 }}>{p.tag}</div>
        ...
        <FadeUp>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: mobile ? "flex-start" : "flex-end", marginBottom: mobile ? 36 : 52, flexWrap: "wrap", gap: 20 }}>
            ...
            <div style={{ textAlign: "right" }}>
              <span style={{ fontFamily: "Georgia,serif", fontSize: "4.5rem", fontWeight: 900, color: "#D4520A", lineHeight: 1, display: "block" }}>{avg}</span>
        ...
        <FadeUp delay={0.2}>
          <div className="review-form" style={{ background: "#181818", padding: "48px clamp(20px,4vw,52px)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 14, marginBottom: 14 }}>
              ...
              <div>
                <label style={{ color: "rgba(255,255,255,0.38)", fontSize: "0.65rem", letterSpacing: "0.18em", textTransform: "uppercase", display: "block", marginBottom: 7 }}>Kota</label>
                <input value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} placeholder="Surabaya" style={inputStyle}
                  onFocus={e => e.target.style.borderColor = "#C9A84C"} onBlur={e => e.target.style.borderColor = "rgba(255,255,255,0.1)"} />
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr", gap: 14, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 6 }}>
              <button onClick={submit} style={{ background: "#D4520A", color: "#fff", border: "none", padding: "12px 32px", fontSize: "0.85rem", fontWeight: 600, letterSpacing: "0.08em", cursor: "pointer", transition: "background 0.2s, transform 0.2s", fontFamily: "inherit" }}
                onMouseEnter={e => { e.target.style.background = "#F06A1A"; e.target.style.transform = "translateY(-2px)" }}
                onMouseLeave={e => { e.target.style.background = "#D4520A"; e.target.style.transform = "translateY(0)" }}>
                Kirim Ulasan
              </button>
    ...
    <section id="contact" style={{ background: "#181818", display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr" }}>
      <FadeUp>
        <div style={{ padding: mobile ? "48px 24px" : "90px clamp(24px,5vw,60px)", borderRight: mobile ? "none" : "1px solid rgba(255,255,255,0.05)" }}>
          ...
          <h2 style={{ fontFamily: "Georgia,serif", fontSize: "clamp(2.2rem,4vw,3.2rem)", fontWeight: 900, color: "#fff", lineHeight: 1.1, marginBottom: 12 }}>
            Siap Melayani <span style={{ color: "#D4520A", fontStyle: "italic" }}>Anda</span>
          </h2>
          ...
          {[
            ...
            { icon: <svg viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" width="20" height="20"><rect x="1" y="3" width="15" height="13" /><polygon points="16 8 20 3 20 16 16 16" /><circle cx="5.5" cy="18.5" r="2.5" /><circle cx="18.5" cy="18.5" r="2.5" /></svg>, label: "Pengiriman", val: "Seluruh Jawa Timur" },
          ].map(({ icon, label, val }) => (
      ...
      <FadeUp delay={0.1}>
        <div style={{ padding: mobile ? "48px 24px" : "90px clamp(24px,5vw,60px)", background: "#0d1a0e", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ width: 60, height: 60, background: "rgba(37,211,102,0.15)", border: "1px solid rgba(37,211,102,0.3)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 28 }}>
            <WA_SVG size={30} color="#25D366" />
          </div>
          ...
          <a href="https://wa.me/6282132310749?text=Halo%20Pak%2C%20saya%20ingin%20menanyakan%20produk%20besi%20dari%20Kureksari" target="_blank" rel="noopener noreferrer"
            ...
            <WA_SVG size={22} color="#fff" /> Chat WhatsApp Sekarang
          </a>
